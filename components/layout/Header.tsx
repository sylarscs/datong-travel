  ...
  return (
    <header className="sticky top-0 z-50 bg-stone-50/95 backdrop-blur-sm border-b border-stone-200">
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        >
          <span className="w-8 h-8 rounded-lg bg-red-800 flex items-center justify-center">
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="px-3 py-2 rounded-md text-sm font-medium text-stone-700 hover:text-red-800 hover:bg-stone-100 transition-colors"
            >
              {link.label}
            </Link>
          ))}
